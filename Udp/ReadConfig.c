...
FORMAT_TYPE gFormatType = {0}; // フォーマット種別 0=関西支社, 1=関東支社, 2=東北支社, 3=名古屋支社
char gVersion[PARAM_WORDS] = {0};
char gIpAddr[32] = {0};			 // ユーザー名（IPアドレス）
int gPortNo;					 // ポート番号
int gSpotNum;					 // 設置地点数
int gLaneNum;					 // 総車線数
int gSpotLaneNum[LANEMAX] = {0}; // 設置地点ごとの車線数
int gDirection[LANEMAX] = {0};	 // 設置地点ごとの方向
int gKp[LANEMAX] = {0};			 // 設置地点ごとのキロポスト
char gRouteCode[LANEMAX][8];	 // 設置地点ごとの路線コード
...
int readConfig(void)
{
	...
	while (fgets(buff, LINE_WORDS, fp) != NULL) {
		...
		// 値をセット
		if (strcmp(title, "FormatType") == 0) { // フォーマット種別
			gFormatType = atoi(param);
			printf("FormatType:%d (0=Kansai, 1=Nagoya, 2=Kanto, 3=Tohoku, 4=Chubu-ohdan-dou)\n", gFormatType);
		}
		else if (strcmp(title, "Version") == 0) { // バージョン
